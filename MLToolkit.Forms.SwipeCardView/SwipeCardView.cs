...
namespace MLToolkit.Forms.SwipeCardView
{
    public class SwipeCardView : ContentView, IDisposable
    {
        ...
        private const float DefaultBackCardScale = 0.99f;

        private const float DefaultCardRotation = 20;

        private const uint DefaultAnimationLength = 150; // Speed of the animations

        private const bool DefaultLoopCards = false;

        private const int DefaultItemIndex = 0;
        ...
        private int _itemIndex; // The last items index added to the stack of the cards
        ...
        public event EventHandler<TappedCardEventArgs> Tapped;
        ...
        private void Setup()
        {
            ...
            for (var i = 0; i < Math.Min(NumCards, ItemsSource.Count); i++)
            {
                ...
                card.IsVisible = true;
                var image = card.FindByName("Image") as Image;
                var label = card.FindByName("Text") as Label;
        ...
        private async void HandleTouchEnd()
        {
            ...
            if (position == DraggingCardPosition.FinishedOverThreshold && SupportedSwipeDirections.IsSupported(direction))
            {
                ...
                if (direction != SwipeCardDirection.Down)
                {
                    ShowNextCard();
                }
            }
        ...
        private void ShowNextCard()
        {
            ...
            if (_itemIndex < ItemsSource.Count)
            {
                ...
                var image = topCard.FindByName("Image") as Image;
        ...
        private void SendTapped(View sender)
        {
            var cmd = TappedCommand;
            if (cmd != null && cmd.CanExecute(TappedCommandParameter))
            {
                cmd.Execute(new TappedCardEventArgs(sender.BindingContext, TappedCommandParameter));
            }

            Tapped?.Invoke(sender, new TappedCardEventArgs(sender.BindingContext, TappedCommandParameter));
            RotateCard();
        }
